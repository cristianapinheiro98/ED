...
/**
 * PriorityQueueNode represents a node in a priority queue containing a comparable object, order, and a priority value.
 *
 */
public class PriorityQueueNode<T> implements Comparable<PriorityQueueNode> {
    ...
    /**
     * Returns the 1 if the current node has higher priority than the given node and -1 otherwise.
     *
    ...
    public int compareTo(PriorityQueueNode obj) {
        ...
        if (priority > temp.getPriority())
            ...
            result = -1;
        else if (order > temp.getOrder())//só chega aqui se as prioridades forem iguais, comparando pela ordem de chegada
            result = 1;
        else
            result = -1;
        return result;
